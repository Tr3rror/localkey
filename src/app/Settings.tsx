        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: () => {
              ...
              haptic('success');
              showFlash(`${result.imported.length} password(s) imported ✓`);
            },
          },
        ],
      );
    ...
    <View style={[s.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Section title="Vault" colors={colors}>
          ...
          {isMasterAdmin && (
            <Row label="Fingerprint unlock" sub="Use biometrics to access your vault"
              onPress={async () => {
                haptic('light');
                try {
                  ...
                  if (!compatible || !enrolled) {
                    Alert.alert('Not available', 'No biometric credentials are enrolled on this device. Please set up fingerprint or face ID in your device settings.');
                    return;
                  }
                  const result = await (await import('expo-local-authentication')).authenticateAsync({
      ...
      <Animated.View
        pointerEvents="none"
        style={[sFlash.toast, { backgroundColor: colors.accent, opacity: flashAnim }]}
      >
        <Text style={[sFlash.txt, { color: colors.background }]}>{flashMsg}</Text>
      </Animated.View>
    </View>
  );
}

const sFlash = StyleSheet.create({
  toast: { position: 'absolute', bottom: 40, left: 32, right: 32, borderRadius: 12, paddingVertical: 14, alignItems: 'center', zIndex: 999 },
