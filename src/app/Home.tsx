...
function AlphabetSidebar({ active, onChange, accentColor, subtextColor }: {
  ...
  const containerH   = useRef(0);  // measured height of the sidebar
  useEffect(() => { activeRef.current = active; }, [active]);

  // The letters are centred inside the container. Total letters block height =
  // ALPHABET.length * LETTER_ITEM_H. The top padding before the first letter is:
  //   (containerH - totalLettersH) / 2
  // We subtract that offset before dividing by LETTER_ITEM_H.
  function letterAt(y: number): string | null {
    const totalH  = ALPHABET.length * LETTER_ITEM_H;
    const topPad  = Math.max(0, (containerH.current - totalH) / 2);
    const adjusted = y - topPad;
    const i = Math.floor(adjusted / LETTER_ITEM_H);
  ...
  return (
    <View
      style={s.alphabetWrap}
      {...pan.panHandlers}
      onLayout={e => { containerH.current = e.nativeEvent.layout.height; }}
    >
      {ALPHABET.map(letter => (
...
export default function Home() {
  ...
  function handleSave() {
    if (!form.label.trim()) { Alert.alert(tr('required'), tr('enterSiteName')); return; }
  ...
  function handleDelete() {
    ...
    Alert.alert('Delete entry', `Delete "${selected.label}"? This cannot be undone.`, [
      ...
      { text: tr('delete'), style: 'destructive', onPress: () => { deletePasswordEntry(userId, selected.id); reload(); closeDrawer(); } },
    ]);
  }
  function handleBack() {
    ...
    <View style={[s.root, { backgroundColor: colors.background }]}>
      ...
      {isHiddenMode && (
        <View style={[s.hiddenBanner, { backgroundColor: colors.accent + '22', borderColor: colors.accent + '33' }]}>
          <Text style={[s.hiddenBannerTxt, { color: colors.accent }]}>{tr('hiddenVaultBanner')}</Text>
        </View>
      )}
          ...
          {searchMode ? (
            ...
            <TouchableOpacity style={[s.searchBar, { backgroundColor: colors.card }]} onPress={enterSearch}>
              ...
              <Text style={[s.searchPlaceholder, { color: colors.subtext }]}>
                {passwords.length > 0 ? (tr('searchEntries') as unknown as Function)(passwords.length) : tr('searchPasswords')}
              </Text>
            </TouchableOpacity>
          )}
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <Text style={[s.emptyTitle, { color: colors.text }]}>
                {passwords.length === 0
                  ? (isHiddenMode ? tr('noHiddenPasswords') : tr('noPasswordsYet'))
                  : tr('noResults')}
              </Text>
          ...
          <Animated.View style={[s.sheet, {
            ...
            <View style={[s.sheetHead, { borderColor: colors.background }]}>
              <View style={s.sheetHeadLeft}>
                ...
                <Text style={[s.sheetTitle, { color: colors.text }]}>
                  {drawerMode === 'add' ? tr('newEntry')
                    : drawerMode === 'edit' ? tr('editEntry')
                    : form.label || tr('newEntry')}
                </Text>
              </View>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.sheetBody}>
              <ScrollView
                ...
                showsVerticalScrollIndicator={false}
              >
                <DrawerField label={tr('siteApp')} value={form.label}
                  editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, label: v }))}
                  placeholder={tr('siteAppHint')}
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('usernameField')} value={form.username}
                  editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, username: v }))}
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('emailField')} value={form.email ?? ''}
                  editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, email: v }))}
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('passwordField')} value={form.password}
                  secureDefault editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, password: v }))}
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('urlField')} value={form.url ?? ''}
                  editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, url: v }))}
                  placeholder="https://"
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('phoneField')} value={form.telefono ?? ''}
                  editable={drawerMode !== 'view'}
                  onChangeText={v => setForm(f => ({ ...f, telefono: v }))}
                  placeholder="+39 000 0000000"
                  bgColor={colors.background} textColor={colors.text} subtextColor={colors.subtext} accentColor={colors.accent} />
                <DrawerField label={tr('notesField')} value={form.notes ?? ''}
                  editable={drawerMode !== 'view'}
                ...
                {drawerMode !== 'view' && (
                  <View style={[s.hiddenRow, { backgroundColor: colors.background }]}>
                    <View style={{ flex: 1 }}>
                      <Text style={[s.hiddenLabel, { color: colors.text }]}>{tr('hiddenEntry')}</Text>
                      <Text style={[s.hiddenSub, { color: colors.subtext }]}>{tr('hiddenEntrySub')}</Text>
                    </View>
                    <Switch
                      value={!!form.isHidden}
                ...
                {drawerMode === 'view' && form.isHidden && (
                  <View style={[s.hiddenBadge, { borderColor: colors.accent + '44', backgroundColor: colors.accent + '11' }]}>
                    <Text style={[s.hiddenBadgeTxt, { color: colors.accent }]}>{tr('hiddenEntryBadge')}</Text>
                  </View>
                )}
              </ScrollView>
            ...
            <View style={[s.sheetFoot, { borderColor: colors.background }]}>
              {drawerMode === 'view' ? (
                ...
                >
                  <Text style={[s.btnPrimaryTxt, { color: colors.background }]}>{tr('editBtn')}</Text>
                </TouchableOpacity>
              ) : (
                <View style={s.footRow}>
                  <TouchableOpacity
                    ...
                    onPress={closeDrawer}
                  >
                    <Text style={[s.btnSecondaryTxt, { color: colors.subtext }]}>{tr('cancel')}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    <Text style={[s.btnPrimaryTxt, { color: colors.background }]}>
                      {drawerMode === 'add' ? tr('saveEntry') : tr('saveChanges')}
                    </Text>
                  </TouchableOpacity>
                </View>
              )}
